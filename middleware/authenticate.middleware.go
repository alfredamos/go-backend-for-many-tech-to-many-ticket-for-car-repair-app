...
func GenerateAccessToken(name, email, userId, role string) (string, error) {
	expireTime := time.Now().Add(time.Second * 15 * 60).Unix()
...
func GenerateRefreshToken(name, email, userId, role string) (string, error) {
	expireTime := time.Now().Add(time.Second * 24 * 7 * 60 * 60).Unix()
...
func ValidateToken(token string, ctx *fiber.Ctx) (TokenJwt, error) {
	//----> Validate token.
	parsedToken, err := validateToken(token)

	//----> Check for error in validating the token
	if err != nil {
		return TokenJwt{}, errors.New(err.Error())
	}

	//----> Get user claims.
	if err = getUserClaims(ctx, parsedToken); err != nil {
		return TokenJwt{}, errors.New(err.Error())
	}

	//----> Get user info.
	userAuth := GetUserIdEmailNameRole(ctx)
